    ...
    def forward(self,
        ...
        if self.flash and seq_len > 1:
            if attention_mask is None or torch.all(attention_mask == 1):
                ...
                causal_mask = torch.triu(torch.full((seq_len, seq_len), float("-inf"), device=xq.device, dtype=xq.dtype), diagonal=1)
                extended_mask = (1.0 - attention_mask.unsqueeze(1).unsqueeze(2)) * float("-inf")
                attn_mask, is_causal = causal_mask.unsqueeze(0) + extended_mask.to(xq.dtype), False
