...
from dst_hooks import DeadNeuronDetector, GNDMonitor
...
# ============================================================
def train_phase1(epoch, loader, iters, rigl_scheduler, gnd_monitor, start_step=0, wandb=None):
    ...
    for step, (X, Y, loss_mask) in enumerate(loader, start=start_step + 1):
        ...
        if (step + 1) % args.phase1_accumulation_steps == 0:
            ...
            # GND监控（在梯度清零前检查，此时梯度已累积完成）
            if step % args.monitor_interval == 0 and step > 0:
                is_sat, ratio, gnorm = gnd_monitor.check_saturation()
                Logger(f'[DST-GND] Step {step}: grad_norm={gnorm:.4f}, ratio={ratio:.6f}, 饱和={is_sat}')
                if is_sat:
                    Logger('[DST] 梯度范数衰减至饱和阈值，提前结束阶段一')
        ...
        # MBE诊断日志已禁用（MBE对小模型失效，不再计算）
        # if step % args.monitor_interval == 0 and step > 0:
        #     _, avg_mbe, _ = mbe_monitor.check_model_mbe()
        #     Logger(f'[DST-MBE-Diag] Step {step}: MBE={avg_mbe:.4f}')

        # 定期保存
        if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            _save_model(f'dst_phase1_{lm_config.hidden_size}')

    return saturated_early
...
if __name__ == "__main__":
    ...
    parser.add_argument("--phase3_epochs", type=int, default=0, help="恢复训练轮数(0=跳过)")
    ...
    parser.add_argument("--monitor_interval", type=int, default=100, help="GND梯度监控间隔(步)")
    # MBE已禁用，不再使用
    # parser.add_argument("--mbe_threshold", type=float, default=0.3, help="MBE饱和阈值(仅诊断)")
    parser.add_argument("--phase1_max_epochs", type=int, default=None, help="阶段一最大轮数上限(默认=phase1_epochs)")
    parser.add_argument("--gnd_threshold", type=float, default=0.1, help="GND梯度范数衰减饱和阈值")
    parser.add_argument("--gnd_patience", type=int, default=3, help="GND连续低于阈值次数忍耐值")
    ...
    # MBE监控已禁用（对小模型hidden_size=512失效，由GND替代）
    # mbe_monitor = MBEMonitor(_get_raw_model(), threshold=args.mbe_threshold)
    gnd_monitor = GNDMonitor(_get_raw_model(), threshold=args.gnd_threshold, patience=args.gnd_patience)

    # 阶段一最大轮数（默认等于phase1_epochs）
    phase1_max_epochs = args.phase1_max_epochs if args.phase1_max_epochs is not None else args.phase1_epochs
    Logger(f'[DST] 阶段一最大轮数: {phase1_max_epochs}, GND饱和阈值: {args.gnd_threshold}, 忍耐次数: {args.gnd_patience}')
    ...
    for epoch in range(args.phase1_epochs):
        ...
        saturated_early = train_phase1(epoch, loader, len(loader), rigl_scheduler, gnd_monitor, 0, wandb)
        if saturated_early:
            Logger('[DST] 阶段一因GND饱和提前终止')
            break

    # 阶段一结束日志
    if saturated_early:
        Logger('[DST] 阶段一: 通过GND饱和检测触发退出')
    elif epoch + 1 >= phase1_max_epochs:
        Logger('[DST] 阶段一: 达到最大轮数限制，梯度范数未衰减至饱和阈值，继续进入阶段二')
    else:
        Logger('[DST] 阶段一: 正常完成所有轮次')
    ...
    # ================================================================
    # 阶段三：恢复与再成长（默认跳过，需手动启用）
    # ================================================================
    if args.phase3_epochs <= 0:
        Logger('\n' + '=' * 60)
        Logger('[DST] phase3_epochs=0，跳过恢复训练阶段')
        Logger('=' * 60)
    else:
        Logger('\n' + '=' * 60)
